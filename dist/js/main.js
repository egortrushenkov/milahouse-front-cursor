...
if (typeof Swiper !== "undefined" && window.SwiperModules) {
  var _window$SwiperModules = window.SwiperModules,
    _Navigation = _window$SwiperModules.Navigation,
    _Pagination = _window$SwiperModules.Pagination,
    _Scrollbar = _window$SwiperModules.Scrollbar,
    _Autoplay = _window$SwiperModules.Autoplay,
    _Grid = _window$SwiperModules.Grid,
    _Thumbs = _window$SwiperModules.Thumbs,
    _FreeMode = _window$SwiperModules.FreeMode;
  Swiper.use([_Navigation, _Pagination, _Scrollbar, _Autoplay, _Grid, _Thumbs, _FreeMode]);
  Swiper.defaults.touchStartPreventDefault = false;
}
function initRooms() {
  var roomsGrid = document.getElementById("roomsGrid");
  if (!roomsGrid) return;
  var activeTab = document.querySelector(".rooms__tab--active");
  var activeCategory = activeTab ? activeTab.dataset.tab : "all";
  var allRooms = roomsGrid.querySelectorAll("[data-room-id]");
  allRooms.forEach(function (room) {
    var roomCategory = room.dataset.roomCategory;
...
function initRoomsTabs() {
  var tabsContainer = document.querySelector(".rooms__tabs");
  if (!tabsContainer) return;
  if (window.innerWidth <= 768 && typeof Swiper !== "undefined") {
    var slider = document.querySelector('[data-slider="rooms-tabs"]');
    if (slider) {
      var swiper = slider.querySelector('[data-slider-swiper="rooms-tabs"]');
      if (swiper) {
        var _ref = window.SwiperModules || {},
          _FreeMode2 = _ref.FreeMode;
        new Swiper(swiper, {
          modules: _FreeMode2 ? [_FreeMode2] : [],
          slidesPerView: "auto",
          spaceBetween: 8,
          freeMode: true
        });
...
function initCounters() {
  ...
  heroForm.addEventListener("click", function (e) {
    ...
    e.preventDefault();
    e.stopPropagation();
    var counterWrapper = target.closest(".hero__form-counter-wrapper");
    if (!counterWrapper) {
      counterWrapper = target.closest(".flex.items-center.gap-2");
    }
    if (!counterWrapper) return;
    var valueEl = counterWrapper.querySelector("[data-counter-value]");
    if (!valueEl) return;
    var formGroup = counterWrapper.closest(".hero__form-group");
    if (!formGroup) {
      formGroup = counterWrapper.closest(".relative");
    }
    var label = (_formGroup = formGroup) === null || _formGroup === void 0 ? void 0 : _formGroup.querySelector("label");
    var maxValue = 15;
    var minValue = 0;
    var value = parseInt(valueEl.value) || 0;
 ...
 */
function getBookingsFromData(element) {
  var _element$dataset, _element$dataset2;
  var roomId = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
  var bookings = [];
  var bookingsData = (element === null || element === void 0 || (_element$dataset = element.dataset) === null || _element$dataset === void 0 ? void 0 : _element$dataset.bookings) || (element === null || element === void 0 || (_element$dataset2 = element.dataset) === null || _element$dataset2 === void 0 ? void 0 : _element$dataset2.roomBookings);
  if (bookingsData) {
    try {
  ...
  if (roomId !== null && roomId !== undefined) {
    bookings = bookings.filter(function (booking) {
      var bookingRoomId = booking.roomId ? String(booking.roomId) : null;
...
function isDateBookedGlobal(date, roomId) {
  var bookings = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
  if (!bookings) {
    bookings = [];
  }
  var dateStr = toISODateGlobal(date);
  return bookings.some(function (booking) {
    var inRange = dateStr >= booking.checkin && dateStr < booking.checkout;
...
function getDisabledDates(bookings, roomId) {
  ...
  for (var _d = new Date(maxDate); _d <= new Date(maxDate.getTime() + 365 * 24 * 60 * 60 * 1000); _d.setDate(_d.getDate() + 1)) {
    disabledDates.push(new Date(_d));
  }
  ...
  if (bookings && bookings.length > 0) {
    bookings.forEach(function (booking) {
      ...
      for (var _d2 = new Date(checkin); _d2 < checkout; _d2.setDate(_d2.getDate() + 1)) {
        disabledDates.push(new Date(_d2));
      }
    });
  }
  return disabledDates;
}
var modalDatePickerApi = null;
function initDatePickers() {
  if (typeof index_es === "undefined") {
    return;
  }
  ...
  function formatDate(date) {
    ...
    var month = MONTHS_RU[date.getMonth()];
    var year = date.getFullYear();
  ...
  function parseDate(str) {
    ...
    if (parts.length >= 3) {
      ...
      var testDate = new Date(currentYear, month, day);
  ...
  function parseAdminDate(str) {
    ...
    var _parts$map = parts.map(function (part) {
  ...
  function createDatePicker(config) {
    var checkinInput = config.checkinInput,
      checkoutInput = config.checkoutInput,
      checkinBtn = config.checkinBtn,
      checkoutBtn = config.checkoutBtn,
      checkinDropdown = config.checkinDropdown,
      checkoutDropdown = config.checkoutDropdown,
      getRoomId = config.getRoomId,
    ...
    var formatOutput = typeof formatValue === "function" ? formatValue : formatDate;
    var parseInput = typeof parseValue === "function" ? parseValue : parseDate;
    var today = getToday();
    ...
    var bookings = dataSource ? getBookingsFromData(dataSource, getRoomId ? getRoomId() : null) : [];
    var disabledDates = getDisabledDates(bookings, getRoomId ? getRoomId() : null);
    var useShortFormat = formatValue !== undefined && formatValue !== formatDate;
    var dateFormat = useShortFormat ? "dd.MM.yyyy" : "d MMMM yyyy";
    var datepickerOptions = {
      ...
      // Контейнер для календаря (hero__form)
    ...
    var datepicker;
    try {
      datepicker = new index_es(checkinInput, datepickerOptions);
      if (!datepicker) {
        return null;
      }
    ...
    return {
      setRange: function setRange(checkin, checkout) {
        if (checkin && checkout) {
  ...
  var heroConfig = {
    ...
    dataSource: heroForm,
    // Источник данных из data-bookings
  ...
  var modalConfig = {
    ...
    getRoomId: function getRoomId() {
      return selectedRoomData ? selectedRoomData.id : null;
    },
  ...
  var adminConfig = {
    ...
    getRoomId: function getRoomId() {
      ...
      var activeTab = document.querySelector(".admin-tabs__btn--active");
      if (activeTab && activeTab.dataset.roomId) {
        return activeTab.dataset.roomId;
      }
      return ((_document$getElementB = document.getElementById("adminRoomInput")) === null || _document$getElementB === void 0 ? void 0 : _document$getElementB.value) || null;
    },
    formatValue: formatAdminDate,
    parseValue: parseAdminDate,
    onSelect: function onSelect() {
      return updateAdminDaysCount();
    },
...
var main_form = document.querySelector(".hero__form");
if (main_form) {
...
function initGallery() {
  var mobileGrid = document.querySelector(".fine__grid--mobile");
  var moreBtn = document.getElementById("fineMoreBtn");
  ...
  if (moreBtn && mobileGrid) {
    moreBtn.addEventListener("click", function (e) {
      ...
      if (window.innerWidth <= 768) {
        ...
        var hiddenItems = mobileGrid.querySelectorAll(".fine__grid-item--hidden");
        ...
        on: {
          load: function load(fancybox, slide) {
            try {
              ...
              var img = slide.contentEl.querySelector("img");
              if (img) {
...
function initAboutDropsParallax() {
  var leftDrop = document.querySelector(".about__drop--left");
  var rightDrop = document.querySelector(".about__drop--right");
  if (!leftDrop || !rightDrop) return;
  var aboutSection = document.querySelector(".about");
  if (!aboutSection) return;
  var ticking = false;
  var lastScrollY = window.scrollY;
  var accumulatedXLeft = 0;
  var accumulatedXRight = 0;
  function updateParallax() {
    ...
    var scrollProgress = Math.max(0, Math.min(1, (windowHeight - sectionTop) / (windowHeight + sectionHeight)));
    var scale = 0.95 + scrollProgress * 0.15 + Math.sin(scrollY * 0.002) * 0.03;
    var translateY = scrollProgress * 20 - 10;
    if (scrollDelta > 0) {
    ...
    leftDrop.style.transform = "translate(".concat(accumulatedXLeft, "px, ").concat(translateY, "px) scale(").concat(scale, ")");
    rightDrop.style.transform = "translate(".concat(accumulatedXRight, "px, ").concat(translateY, "px) scale(").concat(scale, ")");
    ticking = false;
  }
...
function initEntranceAnimations() {
  var aboutSection = document.querySelector(".about");
  var provenceSection = document.querySelector(".provence");
  var observerOptions = {
    threshold: 0.2,
    rootMargin: "0px"
  };
    ...
    entries.forEach(function (entry) {
      if (entry.isIntersecting) {
        if (entry.target.classList.contains("about")) {
          var leftDrop = entry.target.querySelector(".about__drop--left");
          var rightDrop = entry.target.querySelector(".about__drop--right");
          ...
          var leaves = entry.target.querySelectorAll(".provence__leaf");
          var flowers = entry.target.querySelectorAll(".provence__flower");
...
function initAdvantages() {
  var advantagesSwiper = document.querySelector(".advantages-swiper");
  if (!advantagesSwiper || typeof Swiper === "undefined") return;
  try {
    var _ref3 = window.SwiperModules || {},
      _Pagination2 = _ref3.Pagination,
      _Autoplay2 = _ref3.Autoplay;
    new Swiper(advantagesSwiper, {
      modules: [_Pagination2, _Autoplay2],
      slidesPerView: 1,
      spaceBetween: 0,
      loop: true,
      speed: 500,
...
var ATTRACTIONS_VISIBLE_DEFAULT = 6;
...
function getAttractionsFromDom(containerId) {
  var dataEl = document.getElementById(containerId);
  if (!dataEl) return [];
  var imgs = dataEl.querySelectorAll("img");
...
function mergeAttractionsWithDeduplication(topItems, bottomItems) {
  var merged = _toConsumableArray(topItems);
  var topIds = new Set(topItems.map(function (item) {
    return item.id;
  }));
...
function buildAttractionCard(item, modClass) {
  var cls = modClass ? " attraction-item".concat(modClass) : "";
  return '<div class="attraction-item' + cls + '">' + '<div class="attraction-item__img"><img src="' + escapeHtml(item.img) + '" alt="' + escapeHtml(item.title) + '"></div>' + '<div class="attraction-item__content"><div>' + escapeHtml(item.title) + "</div></div>" + "</div>";
}
function buildAttractionsDesktopTop(items) {
  var chunks = [];
  for (var i = 0; i < items.length; i += 3) {
    var slice = items.slice(i, i + 3);
    var cards = slice.map(function (it) {
      return buildAttractionCard(it, "");
    }).join("");
...
function buildAttractionsDesktopBottom(items) {
  ...
  for (var i = 0; i < items.length; i += 3) {
    var slice = items.slice(i, i + 3);
    var lg = slice[0] ? buildAttractionCard(slice[0], " attraction-item--lg") : "";
    var sm1 = slice[1] ? buildAttractionCard(slice[1], " attraction-item--sm") : "";
    var sm2 = slice[2] ? buildAttractionCard(slice[2], " attraction-item--sm") : "";
    var smGroup = sm1 || sm2 ? '<div class="attraction-item--sm--groups">' + sm1 + sm2 + "</div>" : "";
...
function buildAttractionsMobileItems(items) {
  ...
  while (i < items.length) {
    ...
    if (i + 2 <= items.length) {
      html += '<div class="attractions__block attractions__block--pair">' + '<div class="attractions__block-item">' + buildAttractionCard(items[i], "") + "</div>" + '<div class="attractions__block-item">' + buildAttractionCard(items[i + 1], "") + "</div>" + "</div>";
      i += 2;
    } else if (i + 1 <= items.length) {
      html += '<div class="attractions__block attractions__block--pair">' + '<div class="attractions__block-item">' + buildAttractionCard(items[i], "") + "</div>" + "</div>";
      i += 1;
    }
    if (i >= items.length) break;
    // Блок 2: одна на всю ширину
    html += '<div class="attractions__block attractions__block--full">' + '<div class="attractions__block-item">' + buildAttractionCard(items[i], "") + "</div>" + "</div>";
    i += 1;
    if (i >= items.length) break;
    // Блок 3: одна высокая слева + две в столбец справа
    var splitLeft = buildAttractionCard(items[i], "");
    i += 1;
    var splitRight = "";
    if (i < items.length) {
    ...
    html += '<div class="attractions__block attractions__block--split">' + '<div class="attractions__block-split-left">' + splitLeft + "</div>" + '<div class="attractions__block-split-right">' + splitRight + "</div>" + "</div>";
  }
  return html;
}
function initAttractions() {
  // Читаем два отдельных массива из DOM
  var topItems = getAttractionsFromDom("attractionsDataTop");
  var bottomItems = getAttractionsFromDom("attractionsDataBottom");
  if (topItems.length === 0 && bottomItems.length === 0) return;
  var section = document.getElementById("attractions");
  var topWrapper = document.querySelector(".attraction-swiper-top .swiper-wrapper");
  var bottomWrapper = document.querySelector(".attraction-swiper-bottom .swiper-wrapper");
  var mobileGrid = document.querySelector(".attractions__grid--mobile");
  var moreBtn = document.getElementById("attractionsMoreBtn");

  // Десктоп: верхний и нижний слайдеры — дублируем слайды 3× для бесконечной плавной ленты
  var REPEAT_SLIDES = 3;
  if (topWrapper && topItems.length > 0) {
    var topHtml = buildAttractionsDesktopTop(topItems);
    topWrapper.innerHTML = topHtml.repeat(REPEAT_SLIDES);
  }
  if (bottomWrapper && bottomItems.length > 0) {
    var bottomHtml = buildAttractionsDesktopBottom(bottomItems);
  ...
  var mergedItems = mergeAttractionsWithDeduplication(topItems, bottomItems);
  if (mobileGrid) {
    mobileGrid.innerHTML = buildAttractionsMobileItems(mergedItems);
    var blocks = mobileGrid.querySelectorAll(".attractions__block");
    var initialVisible = 3; // первый цикл: pair + full + split
    blocks.forEach(function (block, idx) {
      if (idx >= initialVisible) block.classList.add("attractions__block--hidden");
    });
  }
  var moreWrapper = document.querySelector(".attractions__more-wrapper");
  if (moreWrapper && mobileGrid) {
    var _blocks = mobileGrid.querySelectorAll(".attractions__block");
    if (_blocks.length <= 3) moreWrapper.style.display = "none";
  }
  if (moreBtn && mobileGrid) {
    moreBtn.addEventListener("click", function (e) {
      e.preventDefault();
      ...
      if (window.innerWidth <= 768) {
        mobileGrid.classList.add("show-more");
        mobileGrid.querySelectorAll(".attractions__block--hidden").forEach(function (el) {
...
function initAttractionSliders() {
  var _window$SwiperModules2;
  if (typeof Swiper === "undefined" || window.innerWidth <= 768) return;
  var Autoplay = (_window$SwiperModules2 = window.SwiperModules) === null || _window$SwiperModules2 === void 0 ? void 0 : _window$SwiperModules2.Autoplay;
  if (!Autoplay) return;
  var ATTRACTIONS_SLIDER_SPEED = 12000;
  var commonAttractionOptions = {
    modules: [Autoplay],
    slidesPerView: "auto",
    spaceBetween: 20,
    loop: true,
  ...
  var attractionSwiperTop = document.querySelector(".attraction-swiper-top");
  if (attractionSwiperTop) {
    new Swiper(attractionSwiperTop, _objectSpread({}, commonAttractionOptions));
  }
  var attractionSwiperBottom = document.querySelector(".attraction-swiper-bottom");
  if (attractionSwiperBottom) {
    new Swiper(attractionSwiperBottom, _objectSpread(_objectSpread({}, commonAttractionOptions), {}, {
      autoplay: {
        delay: 0,
        disableOnInteraction: false,
...
function initMobileMenu() {
  ...
  var navBookingBtn = nav.querySelector(".header__nav-booking");
  if (navBookingBtn) {
    navBookingBtn.addEventListener("click", function () {
      closeMenu();
      scrollToHero();
    });
  }
...
function initPhoneMask() {
  var inputs = [document.getElementById("modalPhone"), document.getElementById("leadPhone"), document.getElementById("adminPhone")].filter(Boolean);
  inputs.forEach(applyPhoneMask);
}
var selectedRoomData = null;
var openPriceModalFromRoom = null;
var openStatusModal = null;
...
if (!window.isFormsBound) {
  ...
  document.addEventListener("submit", /*#__PURE__*/function () {
    var _ref4 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee(e) {
      var form, submitBtn, formData, response, result, _t, _t2;
      return _regenerator().w(function (_context) {
        while (1) switch (_context.p = _context.n) {
          case 0:
            form = e.target; // Проверяем, что это форма из модалки лида или бронирования
          ...
          case 1:
            ...
            _context.n = 3;
            return fetch("/ajax/php/submit-handler.php", {
              method: "POST",
              body: formData
            });
          case 3:
          ...
          case 7:
            Oe.close(); // Закрываем текущую форму
...
function initModal() {
  ...
  var overlay = modal === null || modal === void 0 ? void 0 : modal.querySelector(".modal__overlay");
  ...
  // Modal counters
  var adultsMinus = document.getElementById("modalAdultsMinus");
  var adultsPlus = document.getElementById("modalAdultsPlus");
  var adultsValue = document.getElementById("modalAdultsValue");
  var childrenMinus = document.getElementById("modalChildrenMinus");
  var childrenPlus = document.getElementById("modalChildrenPlus");
  var childrenValue = document.getElementById("modalChildrenValue");
  if (adultsMinus && adultsPlus && adultsValue) {
    var maxAdults = 15;
    var minAdults = 0;
    var updateAdultsValue = function updateAdultsValue() {
      var value = parseInt(adultsValue.value) || 0;
    ...
    adultsValue.addEventListener("input", function () {
      updateAdultsValue();
    });
    adultsValue.addEventListener("blur", function () {
      updateAdultsValue();
    });
    adultsValue.addEventListener("keydown", function (e) {
      if (e.key === "Enter") {
        e.preventDefault();
        updateAdultsValue();
        adultsValue.blur();
  ...
  if (childrenMinus && childrenPlus && childrenValue) {
    var maxChildren = 15;
    var minChildren = 0;
    var updateChildrenValue = function updateChildrenValue() {
      var value = parseInt(childrenValue.value) || 0;
    ...
    // Обработка ввода с клавиатуры
    childrenValue.addEventListener("input", function () {
      updateChildrenValue();
    });
    childrenValue.addEventListener("blur", function () {
      updateChildrenValue();
    });
    childrenValue.addEventListener("keydown", function (e) {
      if (e.key === "Enter") {
        e.preventDefault();
        updateChildrenValue();
        childrenValue.blur();
  ...
  openPriceModalFromRoom = function openPriceModalFromRoom() {
    openModal();
  };
  if (form) {
...
function initLeadModal() {
  ...
  var openBtn = document.getElementById("openLeadModal");
...
function initStatusModals() {
  ...
  var successClose = document.getElementById("successModalClose");
  var errorClose = document.getElementById("errorModalClose");
  var successOverlay = successModal === null || successModal === void 0 ? void 0 : successModal.querySelector(".modal__overlay");
  var errorOverlay = errorModal === null || errorModal === void 0 ? void 0 : errorModal.querySelector(".modal__overlay");
  ...
  if (successClose) {
    successClose.addEventListener("click", function () {
      return closeModal(successModal);
    });
  }
  if (errorClose) {
    errorClose.addEventListener("click", function () {
      return closeModal(errorModal);
    });
  }
  if (successOverlay) {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Escape") {
      if (successModal !== null && successModal !== void 0 && successModal.classList.contains("modal--open")) {
        closeModal(successModal);
      }
      if (errorModal !== null && errorModal !== void 0 && errorModal.classList.contains("modal--open")) {
        closeModal(errorModal);
      }
    }
  });
  openStatusModal = function openStatusModal(status) {
    if (status === "error") {
...
function initAdminDeleteModal() {
  ...
  var closeBtn = document.getElementById("adminDeleteModalClose");
  var overlay = modal === null || modal === void 0 ? void 0 : modal.querySelector(".modal__overlay");
  var cancelBtn = modal === null || modal === void 0 ? void 0 : modal.querySelector("[data-admin-delete-cancel]");
  var confirmBtn = modal === null || modal === void 0 ? void 0 : modal.querySelector("[data-admin-delete-confirm]");
  var bookingsBody = document.querySelector("[data-admin-bookings-body]");
  var totalEl = document.querySelector("[data-admin-total]");
  var pendingRow = null;
  ...
  if (confirmBtn) {
    confirmBtn.addEventListener("click", /*#__PURE__*/_asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee2() {
      var response, _t3;
      return _regenerator().w(function (_context2) {
        while (1) switch (_context2.p = _context2.n) {
          ...
          case 1:
            _context2.p = 1;
            _context2.n = 2;
            return fetch("/local/components/kit/admin.booking/templates/remove_el.php", {
              method: "POST",
              headers: {
                "Content-Type": "application/x-www-form-urlencoded"
              },
              body: "id=".concat(encodeURIComponent(pendingBookingId))
            });
          case 2:
          ...
          case 3:
            _context2.p = 3;
            _t3 = _context2.v;
            console.error("Error deleting booking:", _t3);
            alert("Ошибка при удалении брони");
          case 4:
            closeModal();
          case 5:
...
function initRoomModal() {
  ...
  var closeBtn = document.getElementById("roomModalClose");
  var cancelBtn = document.getElementById("roomModalCancel");
  var overlay = modal === null || modal === void 0 ? void 0 : modal.querySelector(".room-modal__overlay");
  var roomCards = document.querySelectorAll(".rooms__card");
  if (!modal) return;
  function openRoomModal(roomData, roomCardElement) {
    ...
    if (modalDatePickerApi && modalDatePickerApi.datepicker && roomCardElement) {
      try {
        var bookings = getBookingsFromData(roomCardElement, roomData.id);
        var disabledDates = getDisabledDates(bookings, roomData.id);
        modalDatePickerApi.datepicker.update({
          disabledDates: disabledDates
        });
      } catch (e) {
    ...
    var titleEl = document.getElementById("roomModalTitle");
    var priceEl = document.getElementById("roomModalPrice");
    var sizeEl = document.getElementById("roomModalSize");
    var capacityEl = document.getElementById("roomModalCapacity");
    var descriptionEl = document.getElementById("roomModalDescription");
    var amenitiesEl = document.getElementById("roomModalAmenities");
    ...
    if (amenitiesEl && roomData.amenities) {
      ...
      roomData.amenities.forEach(function (amenity) {
        var amenityEl = document.createElement("div");
        amenityEl.className = "room-modal__amenity";
        if (amenity.icon) {
          amenityEl.innerHTML = "<img src=\"".concat(amenity.icon, "\" alt=\"").concat(amenity.name, "\">");
        }
  ...
  var bookBtn = document.getElementById("roomModalBook");
  if (bookBtn) {
    bookBtn.addEventListener("click", function () {
      ...
      if (typeof openPriceModalFromRoom === "function") {
        ...
        var openPriceBtn = document.getElementById("openPriceModal");
...
function setupRoomSlider(images) {
  var track = document.querySelector(".room-modal__slider-track");
  var pagination = document.getElementById("roomSliderPagination");
  var prevBtn = document.getElementById("roomSliderPrev");
  var nextBtn = document.getElementById("roomSliderNext");
  if (!track) return;
  ...
  images.forEach(function (img, index) {
    ...
    slide.innerHTML = "<img src=\"".concat(img, "\" alt=\"Room image ").concat(index + 1, "\">");
    track.appendChild(slide);
    if (pagination) {
      var dot = document.createElement("button");
      dot.className = "room-modal__slider-dot ".concat(index === 0 ? "room-modal__slider-dot--active" : "");
      dot.setAttribute("aria-label", "Slide ".concat(index + 1));
...
function updateSlider() {
  ...
  var offset = -roomModalSlider.currentIndex * 100;
  roomModalSlider.track.style.transform = "translateX(".concat(offset, "%)");

  // Update pagination
  if (roomModalSlider.pagination) {
    var dots = roomModalSlider.pagination.querySelectorAll(".room-modal__slider-dot");
...
var roomModalDatePickerApi = null;
function setupRoomModalDatePicker(roomCardElement, roomId) {
  if (!window.AirDatepicker) return;
  var bookings = roomCardElement ? getBookingsFromData(roomCardElement, roomId) : [];
  var disabledDates = getDisabledDates(bookings, roomId);
  var today = new Date();
  ...
  var datepickerOptions = {
    ...
    onSelect: function onSelect(_ref6) {
      var date = _ref6.date,
        formattedDate = _ref6.formattedDate,
        datepicker = _ref6.datepicker;
      // Обновляем значения в форме бронирования, если она открыта
      var modalCheckinInput = document.getElementById("modalCheckin");
      var modalCheckoutInput = document.getElementById("modalCheckout");
  ...
  var container = document.getElementById("roomModalCalendarContainer");
  if (!container) return;

  // Очищаем контейнер перед созданием нового календаря
  container.innerHTML = "";
  var datepicker = new window.AirDatepicker(container, datepickerOptions);
...
function syncRoomCalendarFromForm() {
  ...
  var checkinDate = parseFormDate(modalCheckinInput.value);
  var checkoutDate = parseFormDate(modalCheckoutInput.value);
...
function initCookieBanner() {
  ...
  var storageKey = "milahouse_cookie_consent";
  var hasConsent = localStorage.getItem(storageKey) === "true";
  if (!hasConsent) {
...
function initHeaderNavFixed() {
  ...
  var onScroll = function onScroll() {
    ...
    var trigger = headerTop.offsetHeight || 92;
...
function initNotFoundBack() {
  var backBtn = document.getElementById("notFoundBack");
  if (!backBtn) return;
...
function initPrivacyBack() {
  var backBtn = document.getElementById("privacyBack");
  if (!backBtn) return;
  backBtn.addEventListener("click", function () {
    if (window.history.length > 1) {
      window.history.back();
...
function initAdminSelect() {
  var selects = document.querySelectorAll("[data-admin-select]");
  selects.forEach(function (select) {
    var control = select.querySelector(".admin-select__control");
    var list = select.querySelector(".admin-select__list");
    var valueEl = select.querySelector("[data-admin-select-value]");
    ...
    control.addEventListener("click", function (e) {
      e.preventDefault();
      e.stopPropagation();
      var isOpen = select.classList.toggle("admin-select--open");
      control.setAttribute("aria-expanded", isOpen ? "true" : "false");
    });
    list.addEventListener("click", function (e) {
      var option = e.target.closest(".admin-select__option");
      if (!option) return;
...
function initAdminCounters() {
  var counters = document.querySelectorAll("[data-admin-counter]");
  counters.forEach(function (counter) {
    var valueEl = counter.querySelector(".admin-counter__value");
    var minusBtn = counter.querySelector('[data-action="minus"]');
    var plusBtn = counter.querySelector('[data-action="plus"]');
...
function updateAdminDaysCount() {
  var daysEl = document.querySelector("[data-admin-days]");
  var checkinInput = document.getElementById("adminCheckin");
  var checkoutInput = document.getElementById("adminCheckout");
  if (!daysEl || !checkinInput || !checkoutInput) return;
  var parseDate = function parseDate(value) {
    if (!value) return null;
    var parts = value.split(".");
    if (parts.length !== 3) return null;
    var _parts$map3 = parts.map(function (part) {
        return parseInt(part, 10);
      }),
      _parts$map4 = _slicedToArray(_parts$map3, 3),
      day = _parts$map4[0],
      month = _parts$map4[1],
      year = _parts$map4[2];
    if (!day || !month || !year) return null;
  ...
  var checkin = parseDate(checkinInput.value);
  var checkout = parseDate(checkoutInput.value);
  if (!checkin || !checkout || checkout <= checkin) {
    daysEl.textContent = "0";
    return;
  }
  var diff = Math.round((checkout - checkin) / (1000 * 60 * 60 * 24));
  daysEl.textContent = String(diff);
}
function initAdminPanel() {
  var adminRoot = document.querySelector(".admin");
  if (!adminRoot) return;
  var tabs = adminRoot.querySelectorAll("[data-admin-room-tab]");
  var calendarsEl = adminRoot.querySelector("[data-admin-calendars]");
  var bookingsBody = adminRoot.querySelector("[data-admin-bookings-body]");
  var totalEl = adminRoot.querySelector("[data-admin-total]");
  var yearEl = adminRoot.querySelector("[data-admin-year]");
  var checkinInput = document.getElementById("adminCheckin");
  var checkoutInput = document.getElementById("adminCheckout");
  ...
  var getActiveRoomId = function getActiveRoomId() {
    var active = adminRoot.querySelector(".admin-tabs__btn--active");
    return active ? active.dataset.roomId : null;
  };
  var setStaticCalendar = function setStaticCalendar(roomId) {
    if (!calendarsEl || calendarsEl.dataset.adminStatic !== "true") return;
    var calendars = Array.from(calendarsEl.querySelectorAll(".admin-calendar"));
    if (!calendars.length) return;
    var activeIndex = Math.max(0, Number(roomId || 1) - 1);
    calendars.forEach(function (calendar, index) {
      calendar.classList.toggle("is-active", index === activeIndex);
    });
  };
  var getBookingsForRoom = function getBookingsForRoom(roomId) {
    // Сначала пытаемся получить данные из data-атрибутов табов
    var activeTab = adminRoot.querySelector(".admin-tabs__btn--active");
    if (activeTab && activeTab.dataset.roomBookings) {
      try {
        var bookings = JSON.parse(activeTab.dataset.roomBookings);
        if (roomId) {
          return bookings.filter(function (booking) {
            return String(booking.roomId || "") === String(roomId);
          });
        }
        return bookings;
      } catch (e) {}
  ...
  var renderCalendars = function renderCalendars(roomId) {
    ...
    var months = Array.from({
      ...
      return new Date(currentYear, idx, 1);
    });

    // Получаем бронирования для текущего номера
    var bookings = getBookingsForRoom(roomId);
    calendarsEl.innerHTML = months.map(function (monthDate) {
      var year = monthDate.getFullYear();
      var month = monthDate.getMonth();
      var firstDay = new Date(year, month, 1);
      var daysInMonth = new Date(year, month + 1, 0).getDate();
      var offset = (firstDay.getDay() + 6) % 7; // Понедельник = 0

      // Получаем последние дни предыдущего месяца
      var prevMonthDays = new Date(year, month, 0).getDate();

      // Получаем количество дней для следующего месяца
      var totalDaysShown = offset + daysInMonth;
      var nextMonthDays = totalDaysShown <= 35 ? 35 - totalDaysShown : 42 - totalDaysShown;
      var daysHtml = "";

      // Дни предыдущего месяца
      for (var i = offset - 1; i >= 0; i--) {
        var day = prevMonthDays - i;
        daysHtml += "<div class=\"admin-calendar__day admin-calendar__day--muted\">".concat(day, "</div>");
      }
      ...
      var _loop = function _loop() {
        ...
        date.setHours(0, 0, 0, 0);
        var dateStr = "".concat(year, "-").concat(String(month + 1).padStart(2, "0"), "-").concat(String(dayNumber).padStart(2, "0"));
        ...
        var className = "admin-calendar__day";
        if (isBooked) {
          className += " admin-calendar__day--booked";
        } else if (isFuture) {
          className += " admin-calendar__day--free";
        }
        daysHtml += "<div class=\"".concat(className, "\" data-date=\"").concat(dateStr, "\">").concat(dayNumber, "</div>");
      };
      ...
      for (var _i = 1; _i <= nextMonthDays; _i++) {
        daysHtml += "<div class=\"admin-calendar__day admin-calendar__day--muted\">".concat(_i, "</div>");
      }
      return "\n        <div class=\"admin-calendar\">\n          <div class=\"admin-calendar__title\">".concat(monthNames[month], ", ").concat(year, "</div>\n          <div class=\"admin-calendar__weekdays\">\n            ").concat(weekdays.map(function (day) {
        return "<div class=\"admin-calendar__weekday\">".concat(day, "</div>");
      }).join(""), "\n          </div>\n          <div class=\"admin-calendar__days\">\n            ").concat(daysHtml, "\n          </div>\n        </div>\n      ");
    }).join("");
  };
  var renderBookings = function renderBookings(roomId) {
    ...
    var totalSum = 0;
    bookingsBody.innerHTML = bookings.map(function (booking, index) {
      var _booking$guest, _booking$guests;
      var checkin = booking.checkin ? new Date("".concat(booking.checkin, "T00:00:00")) : null;
      var checkout = booking.checkout ? new Date("".concat(booking.checkout, "T00:00:00")) : null;
      var nights = checkin && checkout ? Math.ceil((checkout - checkin) / (1000 * 60 * 60 * 24)) : 0;
      var pricePerNight = Number(booking.price || 0);
      var totalPrice = Number(booking.total || booking.totalPrice || booking.priceTotal || pricePerNight || 0);
      totalSum += totalPrice || 0;

      // Поддержка разных форматов данных
      var guestName = ((_booking$guest = booking.guest) === null || _booking$guest === void 0 ? void 0 : _booking$guest.name) || booking.guest || booking.name || "—";
      var guestPhone = booking.phone || "—";
      var guestsCount = (_booking$guests = booking.guests) !== null && _booking$guests !== void 0 && _booking$guests.adults ? "".concat(booking.guests.adults + (booking.guests.children || 0)) : booking.guests || booking.guestsCount || "—";
      var bitrixPathTemplate = "/local/templates/main";
      return "\n        <div class=\"admin-table__row\" data-booking-id=\"".concat(booking.id || index + 1, "\">\n          <span class=\"admin-table__id\">\n            <img src=\"").concat(bitrixPathTemplate, "/img/svg/icon-change.svg\" alt=\"\" class=\"admin-table__edit-icon\">\n            ").concat(booking.id || index + 1, "\n          </span>\n          <span>").concat(guestName, "</span>\n          <span><a href=\"tel:").concat(guestPhone, "\">").concat(guestPhone, "</a></span>\n          <span>").concat(checkin ? formatShortDate(checkin) : "—", "</span>\n          <span>").concat(checkout ? formatShortDate(checkout) : "—", "</span>\n          <span>").concat(guestsCount, "</span>\n          <span>").concat(formatNumber(totalPrice), "</span>\n          <span>").concat(booking.comment || "—", "</span>\n          <button type=\"button\" class=\"admin-table__delete\" aria-label=\"\u0423\u0434\u0430\u043B\u0438\u0442\u044C\" data-admin-delete data-booking-id=\"").concat(booking.id || index + 1, "\">\n            <img src=\"").concat(bitrixPathTemplate, "/img/svg/icon-admin-delete.svg\" alt=\"\">\n          </button>\n        </div>\n      ");
    }).join("");
  ...
  tabs.forEach(function (tab) {
    tab.addEventListener("click", function () {
      tabs.forEach(function (item) {
        return item.classList.remove("admin-tabs__btn--active");
      });
      tab.classList.add("admin-tabs__btn--active");
      var roomId = tab.dataset.roomId || null;
...
function initPreloader() {
  ...
  if (!preloader) return;
  var minDisplayMs = 400; // минимальное время показа, чтобы не было мелькания
...
Oe.defaults.mainClass = "fancybox-custom";
Oe.defaults.trapFocus = false;
Oe.defaults.autoFocus = false;
Oe.defaults.placeFocusBack = false;
...
Oe.defaults.on = {
  ...
  reveal: function reveal() {
    ...
    setTimeout(function () {
      var content = document.querySelector(".fancybox__content");
      if (content) {
        var phoneInputs = content.querySelectorAll('input[type="tel"]');
...
if (!window.modalCountersGlobalHandlerInitialized) {
  ...
  document.addEventListener("click", function (e) {
    ...
    var modalContent = target.closest(".fancybox__content, .modal__content");
    if (!modalContent) return;
    if (target.matches("[data-counter-minus], .modal__counter-btn--minus, [data-counter-plus], .modal__counter-btn--plus")) {
      e.preventDefault();
      e.stopPropagation();
      var wrapper = target.closest(".modal__form-counter-group") || target.closest(".room-modal__counter-group") || target.closest(".modal__counter");
      if (!wrapper) return;
      var valueEls = wrapper.querySelectorAll("[data-counter-value], .modal__counter-value");
      if (!valueEls || valueEls.length === 0) return;

      // Все счетчики имеют максимум 15
      var maxValue = 15;
      var minValue = 0;
      var value = parseInt(valueEls[0].value) || 0;
...
function initModalContent() {
  ...
  var roomModalSlider = content.querySelector(".room-modal__slider");
  if (roomModalSlider && typeof Swiper !== "undefined") {
    try {
      var _ref7 = window.SwiperModules || {},
        _Navigation2 = _ref7.Navigation,
        _Pagination3 = _ref7.Pagination;
      new Swiper(roomModalSlider, {
        modules: [_Navigation2, _Pagination3],
        slidesPerView: 1,
        spaceBetween: 0,
        loop: true,
        navigation: {
  ...
  var calendarContainer = content.querySelector(".room-modal__calendar-container");
  if (calendarContainer && typeof index_es !== "undefined") {
    try {
      ...
      var bookingsData = calendarContainer.dataset.roomBookings;
      var bookings = [];
      if (bookingsData) {
        try {
          bookings = JSON.parse(bookingsData);
      ...
      window.roomModalFancyboxDatePicker = new index_es(calendarContainer, {
        inline: true,
        range: true,
        multipleDates: false,
        minDate: new Date(),
        ...
        onSelect: function onSelect(_ref8) {
          var date = _ref8.date,
            formattedDate = _ref8.formattedDate;
          // Save selected dates
          if (Array.isArray(date) && date.length === 2) {
        ...
        onRenderCell: function onRenderCell(_ref9) {
          var date = _ref9.date,
            cellType = _ref9.cellType;
          if (cellType === "day") {
  ...
  var bookBtn = content.querySelector("#roomModalBook");
  if (bookBtn) {
    // Удаляем старые обработчики перед добавлением нового
    var newBookBtn = bookBtn.cloneNode(true);
...
function initFormDataHandlers() {
  ...
  var heroForm = document.querySelector(".hero__form");
  if (heroForm) {
    var submitBtn = heroForm.querySelector(".hero__form-submit");
    if (submitBtn) {
      submitBtn.addEventListener("click", function (e) {
        ...
        var checkinInput = heroForm.querySelector("#checkin-input");
        var checkoutInput = heroForm.querySelector("#checkout-input");
        var counterValues = heroForm.querySelectorAll("[data-counter-value]");
        ...
        var heroBookings = heroForm.dataset.bookings || "[]";
        searchParams.append("bookings", heroBookings);

        // Обновляем data-src атрибут для открытия диалога с параметрами
        var originalSrc = submitBtn.dataset.src || "ajax/dialogs/lead-modal.php";
        var baseSrc = originalSrc.split("?")[0]; // Убираем старые параметры если есть
        var queryString = searchParams.toString();
        submitBtn.dataset.src = "".concat(baseSrc, "?").concat(queryString);
  ...
  roomCards.forEach(function (card) {
    card.addEventListener("click", function (e) {
      ...
      var capacity = card.dataset.roomCapacity;
      var roomBookings = card.dataset.roomBookings || "[]";

      // Получаем названия комнаты из карточки
      var roomTitleEl = card.querySelector("[data-room-title], .rooms__card-title");
      var roomName = roomTitleEl ? roomTitleEl.textContent.trim() : "";
      ...
      var counterValues = document.querySelectorAll(".hero__form [data-counter-value]");
      ...
      // Обновляем data-src атрибут для открытия диалога с параметрами
      var originalSrc = card.dataset.src || "";
      var baseSrc = originalSrc.split("?")[0]; // Убираем старые параметры если есть
      var queryString = searchParams.toString();
      card.dataset.src = "".concat(baseSrc, "?").concat(queryString);
...
function initSliders() {
  ...
  // Rooms tabs slider
  var roomsTabsSlider = document.querySelector('[data-slider="rooms-tabs"]');
  if (roomsTabsSlider) {
    var swiper = roomsTabsSlider.querySelector('[data-slider-swiper="rooms-tabs"]');
    if (swiper) {
      var _ref0 = window.SwiperModules || {},
        _FreeMode3 = _ref0.FreeMode;
      new Swiper(swiper, {
        modules: _FreeMode3 ? [_FreeMode3] : [],
        slidesPerView: 2.1,
        spaceBetween: 8,
        freeMode: {
          enabled: true,
          sticky: false,
          // Отключает привязку к краям слайдов
          momentum: true,
          // Добавляет инерцию при прокрутке
          momentumBounce: false // Отключает отскок в конце
        }
      });

      // Handle tab clicks
      var tabs = roomsTabsSlider.querySelectorAll("[data-tab]");
      tabs.forEach(function (tab) {
        tab.addEventListener("click", function () {
          tabs.forEach(function (t) {
  ...
  var fineSwiper = document.querySelector(".fine-swiper");
  ...
  var advantagesSwiper = document.querySelector(".advantages-swiper");
  if (advantagesSwiper) {
    var pagination = document.querySelector(".advantages-pagination");
    new Swiper(advantagesSwiper, {
  ...
  var attractionsSlider = document.querySelector('[data-slider="attractions"]');
  if (attractionsSlider && window.innerWidth > 768) {
    var _swiper = attractionsSlider.querySelector('[data-slider-swiper="attractions"]');
    if (_swiper) {
      new Swiper(_swiper, {
        slidesPerView: "auto",
        spaceBetween: 20,
