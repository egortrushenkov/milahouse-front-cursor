...
function getBookingsFromData(element, roomId = null) {
  ...
  const bookingsData =
    element?.dataset?.bookings || element?.dataset?.roomBookings;
...
function getDisabledDates(bookings, roomId) {
  ...
  for (
    ...
    d <= new Date(maxDate.getTime() + 365 * 24 * 60 * 60 * 1000);
...
function initDatePickers() {
  ...
  function createDatePicker(config) {
    ...
    const formatOutput =
      typeof formatValue === "function" ? formatValue : formatDate;
    const parseInput =
      typeof parseValue === "function" ? parseValue : parseDate;
    const today = getToday();
    ...
    const bookings = dataSource
      ? getBookingsFromData(dataSource, getRoomId ? getRoomId() : null)
      : [];
    const disabledDates = getDisabledDates(
      bookings,
      getRoomId ? getRoomId() : null,
    );

    const useShortFormat =
      formatValue !== undefined && formatValue !== formatDate;
  ...
  if (moreBtn && mobileGrid) {
    moreBtn.addEventListener("click", function (e) {
      ...
      if (window.innerWidth <= 768) {
        ...
        const hiddenItems = mobileGrid.querySelectorAll(
          ".fine__grid-item--hidden",
        );
...
function initAboutDropsParallax() {
  ...
  function updateParallax() {
    ...
    const scale =
      0.95 + scrollProgress * 0.15 + Math.sin(scrollY * 0.002) * 0.03;
    ...
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        if (entry.target.classList.contains("about")) {
          ...
          if (
            leftDrop &&
            !leftDrop.classList.contains("about__drop--animated")
          ) {
            leftDrop.classList.add("about__drop--animated");
          }
          if (
            rightDrop &&
            !rightDrop.classList.contains("about__drop--animated")
          ) {
...
function buildAttractionCard(item, modClass) {
  ...
  return (
    ...
    cls +
    '">' +
    '<div class="attraction-item__img"><img src="' +
    escapeHtml(item.img) +
    '" alt="' +
    escapeHtml(item.title) +
    '"></div>' +
    '<div class="attraction-item__content"><div>' +
    escapeHtml(item.title) +
    "</div></div>" +
    "</div>"
  );
...
function buildAttractionsDesktopTop(items) {
  ...
  for (let i = 0; i < items.length; i += 3) {
    ...
    chunks.push(
      '<div class="swiper-slide attraction-grid">' + cards + "</div>",
    );
  }
  return chunks.join("");
}
...
function buildAttractionsDesktopBottom(items) {
  ...
  for (let i = 0; i < items.length; i += 3) {
    const slice = items.slice(i, i + 3);
    const lg = slice[0]
      ? buildAttractionCard(slice[0], " attraction-item--lg")
      : "";
    const sm1 = slice[1]
      ? buildAttractionCard(slice[1], " attraction-item--sm")
      : "";
    const sm2 = slice[2]
      ? buildAttractionCard(slice[2], " attraction-item--sm")
      : "";
    const smGroup =
      sm1 || sm2
        ...
        : "";
    chunks.push(
      '<div class="swiper-slide attraction-grid">' + lg + smGroup + "</div>",
    );
  }
  return chunks.join("");
}
  ...
  while (i < items.length) {
    ...
    if (i + 2 <= items.length) {
      html +=
        ...
        '<div class="attractions__block-item">' +
        buildAttractionCard(items[i], "") +
        "</div>" +
        '<div class="attractions__block-item">' +
        buildAttractionCard(items[i + 1], "") +
        "</div>" +
        "</div>";
      i += 2;
    ...
    html +=
      ...
      '<div class="attractions__block-split-left">' +
      splitLeft +
      "</div>" +
      '<div class="attractions__block-split-right">' +
      splitRight +
      "</div>" +
      "</div>";
  }
...
function initAttractions() {
  ...
  const topWrapper = document.querySelector(
    ".attraction-swiper-top .swiper-wrapper",
  );
  const bottomWrapper = document.querySelector(
    ".attraction-swiper-bottom .swiper-wrapper",
  );
  ...
  if (mobileGrid) {
    ...
    blocks.forEach((block, idx) => {
      if (idx >= initialVisible)
        block.classList.add("attractions__block--hidden");
    });
  }
  ...
  if (moreBtn && mobileGrid) {
    moreBtn.addEventListener("click", function (e) {
      ...
      if (window.innerWidth <= 768) {
        mobileGrid.classList.add("show-more");
        mobileGrid
          .querySelectorAll(".attractions__block--hidden")
          .forEach((el) => {
...
function initAttractionSliders() {
  ...
  const attractionSwiperBottom = document.querySelector(
    ".attraction-swiper-bottom",
  );
          ...
          if (pendingRow) {
            ...
            if (bookingsBody && bookingsBody.children.length === 0) {
              bookingsBody.innerHTML =
                '<div class="admin-table__empty">Нет броней</div>';
              if (totalEl) {
...
function setupRoomSlider(images) {
  ...
  if (prevBtn) {
    prevBtn.addEventListener("click", () => {
      roomModalSlider.currentIndex =
        (roomModalSlider.currentIndex - 1 + images.length) % images.length;
      updateSlider();
    });
  }
  ...
  if (nextBtn) {
    nextBtn.addEventListener("click", () => {
      roomModalSlider.currentIndex =
        (roomModalSlider.currentIndex + 1) % images.length;
      updateSlider();
    });
  }
}
...
function updateSlider() {
  ...
  if (roomModalSlider.pagination) {
    const dots = roomModalSlider.pagination.querySelectorAll(
      ".room-modal__slider-dot",
    );
...
function setupRoomModalDatePicker(roomCardElement, roomId) {
  ...
  const bookings = roomCardElement
    ? getBookingsFromData(roomCardElement, roomId)
    : [];
...
function initHeaderBookingScroll() {
  const buttons = document.querySelectorAll(".header__booking-btn, .header__nav-booking");
  buttons.forEach(btn => {
...
function initAdminPanel() {
  ...
  const setStaticCalendar = (roomId) => {
    ...
    const calendars = Array.from(
      calendarsEl.querySelectorAll(".admin-calendar"),
    );
      ...
      try {
        ...
        if (roomId) {
          return bookings.filter(
            (booking) => String(booking.roomId || "") === String(roomId),
          );
        }
        return bookings;
      } catch (e) {}
  ...
  const renderBookings = (roomId) => {
    ...
    bookingsBody.innerHTML = bookings
      .map((booking, index) => {
        ...
        const guestName =
          booking.guest?.name || booking.guest || booking.name || "—";
        ...
        const bitrixPathTemplate = "/local/templates/main";
...
function initSliders() {
  ...
  if (roomsTabsSlider) {
    const swiper = roomsTabsSlider.querySelector(
      '[data-slider-swiper="rooms-tabs"]',
    );
    if (swiper) {
      const { FreeMode } = window.SwiperModules || {};
      new Swiper(swiper, {
  ...
  const attractionsSlider = document.querySelector(
    '[data-slider="attractions"]',
  );
  if (attractionsSlider && window.innerWidth > 768) {
    const swiper = attractionsSlider.querySelector(
      '[data-slider-swiper="attractions"]',
    );
    if (swiper) {
      new Swiper(swiper, {
        slidesPerView: "auto",
